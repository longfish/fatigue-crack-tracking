'''
Extract the crack geometry from a stack of cross-section images
Usage: 
    Put the script into the parent path of all image folders
'''
...
def main():
    ...
    for stepf in stepfolders:
        ...
        filter_dir = os.path.join(cwd, stepf+"_filtered")
        if(not os.path.isdir(filter_dir)):
            os.mkdir(filter_dir)

        # cross-section images of this step
        imgs = os.listdir(step_dir)
        img_ref = cv.imread(os.path.join(
            step_dir, imgs[0]), 0)  # to get image size

        # create blank images into the folder
        img_pattern = Path(imgs[0]).stem[:-4]
        blank = np.zeros(img_ref.shape[:2], np.uint8)
        for i in range(1640):
            cv.imwrite(os.path.join(
                filter_dir, img_pattern+str(i+1).zfill(4)+".png"), blank)

        # loop the cross-section images in the current folder
        for img in imgs:
            img_array = cv.imread(os.path.join(
                step_dir, img), 0)  # grayscale image
            crack = image_filter(img_array, 12, 98)  # perform the filter

            # store the contour circle size
            cx, cy, r = cross_section_contour(img_array, 15)
            ...
            '''
            # extract the most significant crack contour(s)
            crack_num = 1
            crack_hub = cracks_extraction(crack, (cx, cy, r), crack_num)
            ...
            '''
            # store the final image into the directory
            cv.imwrite(os.path.join(filter_dir, Path(img).stem+".png"), crack)
