...
def image_filter(img, h, th_g):
    ...
    th1 = cv.fastNlMeansDenoising(gaus, None, h, 7, 21)

    # apply Otsu's binarization (may be more stable)
    # _, th1 = cv.threshold(th1, th_g, 255, cv.THRESH_BINARY+cv.THRESH_OTSU)

    # apply global threshhold (may be more accurate)
    _, th1 = cv.threshold(th1, th_g, 255, cv.THRESH_BINARY)
